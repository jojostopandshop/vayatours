...
// - Top-bar is locked/visible while any sidebar or search is open (body[data-sidebar-open]="true")
// - Prevents auto-hide while sidebar is open or while hovering the top-bar
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function closeSidebarById(id) {
    ...
    // Slight delay to avoid flicker when moving between menus
    setTimeout(() => {
  ...
  function toggleMenu(id) {
    if (isHoverable) {
      // Desktop uses hover; ignore clicks
      return;
    }
  ...
  // Ensure side-bar-item respects data-height
  document.querySelectorAll(".side-bar-item").forEach((item) => {
    const h = item.dataset.height;
    if (h) item.style.height = h;
  });

  // Attach handlers to toggle buttons (buttons with inline onclick="toggleMenu('id')")
  ...
  if (isHoverable) {
    ...
    // Touch devices: keep click behavior but prevent immediate document click closure
  ...
  (function initHeroCarousel2() {
    ...
    // store interval id in case cleanup needed
